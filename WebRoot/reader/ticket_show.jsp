...
<br> <br> <br>
<hr>
<h3 align="center"><a href="${pageContext.request.contextPath}/ticketShowGo.do?key=no">待处理</a>/<a href="${pageContext.request.contextPath}/ticketShowGo.do?key=yes">已处理</a></h3>
<br><br>
<div style="width: 1000px;text-align: center;position: relative;left: 100px">
    <div class="panel panel-default" >
        <%
        List<Ticket> ticket=(List<Ticket>)request.getAttribute("ticket");
        List<String> name=(List<String>)request.getAttribute("ticketname");
        for(int i=0;i<ticket.size();i++){ %>
		<div class="panel-heading">
			<h4 class="panel-title">
				<a data-toggle="collapse" data-parent="#accordion" 
				   href="#collapseOne">
					罚单编号：<%=ticket.get(i).getFineid()%>&nbsp;&nbsp;罚单类型：<%=name.get(i)%>&nbsp;&nbsp;罚金:<%=ticket.get(i).getTotalfineprice() %>
				</a>
			</h4>
		</div>
		<div id="collapseOne" class="panel-collapse collapse in">
			<div class="panel-body">
				详情：<%=ticket.get(i).getFinedetail() %><br>
				<%if(request.getAttribute("old")==null) {%>
				<a href="reader/pay.jsp?fineid=<%=ticket.get(i).getFineid()%>">支付罚金</a><%} %>
			</div>
		</div><%} %>
	</div>
</div>
